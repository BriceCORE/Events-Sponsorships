...
A running request times out after 35 minutes if the worker cannot finish or report its result. You can then request another check. A queued request depends on GitHub Actions being enabled and able to run; it does not run in the browser. If it stays queued, check the workflow's Actions history and repository scheduling status. The queue stores the most recent 30 requests separately from event plans and expenses. Only the trusted GitHub worker can claim and finish them.
